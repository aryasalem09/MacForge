...
| --- | --- | --- | --- |
| 1. Collapsed notch pill | Implemented, requires real screenshot QA | `NotchPanelLayout` keeps the actual `NSPanel` top-anchored while drawing a top-merged black shell and lower safe content. Force Attached Notch Test Mode and Calibration Mode isolate and correct placement. | It cannot draw into hidden camera pixels; exact visual fit may need user calibration on some MacBook models. |
| 2. Hover/click/swipe expansion | Implemented | Hover uses a delayed state machine to prevent flicker, click can pin expanded state, and swipe gestures expand/collapse or control supported media. | Trackpad gesture recognition is app-panel local, not an OS gesture hook. |
...
| 11. Customization | Implemented | Live Island settings cover source toggles, privacy mode, artwork, media persistence, downloads folder, diagnostics, test providers, attached notch offsets, shell height, drag calibration, force visual test mode, layout repair, hard visual reset, and geometry debug copy. Existing island size/material/hover/click settings remain. | No private system HUD replacement or OS-level media API toggle. |
